...
/**
 * For logger output<br/>
 * To avoid jar conflict, default use JDK log, if found commons log jar, use it, if found Log4j jar use it..., by this
 * way this project has no dependency to any logger jar.
 *
 * @author Yong Zhu
 * @since 2.4.2
 */
public class BeanBoxLogger {
	private Object commonLogger;
	private Method commonLoggerInfoMethod;
	private Method commonLoggerErrorMethod;
	private Logger jdkLogger;

	public BeanBoxLogger(Class<?> targetClass) {
		if (targetClass == null)
			throw new AssertionError("BeanBoxLogger error: targetClass can not be null.");
		try {
	...
	public static BeanBoxLogger getLog(Class<?> targetClass) {
		return new BeanBoxLogger(targetClass);
	}
